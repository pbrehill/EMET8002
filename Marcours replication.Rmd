...
Y <- data %>%
  select(contains('z_') & contains('_08') & -contains('_08m'))
...
t.tests_leaves <- valid_samples %>%
  map(function (x) {
    ...
    t.test(leaf_treated$`forest_outputs$z_all_08$Y.orig`,
           leaf_control$`forest_outputs$z_all_08$Y.orig`)
  })
...
```{r}
finished_samples <- baum_trees$z_all_08$nodes %>% 
  map(~.x$samples) %>%
  filter_valid_leaves()
...
```{r}
prune_to_size <- function(tree, data, min.group.size = 30) {
  ...
  all_right_size <- tree$nodes %>%
    map(~.x$samples) %>%
    filter_valid_leaves() %>%
    map_lgl(~length(.x) > 29)
  
  while (!all_right_size) {
